...
gohr_m7 = np.load('./mean_std/data_wrong_key_mean_7r.npy')
gohr_s7 = np.load('./mean_std/data_wrong_key_std_7r.npy'); gohr_s7 = 1.0/gohr_s7
gohr_m6 = np.load('./mean_std/data_wrong_key_mean_6r.npy')
gohr_s6 = np.load('./mean_std/data_wrong_key_std_6r.npy'); gohr_s6 = 1.0/gohr_s6
...
def verifier_search(cts, best_guess, use_n = 64, net = gohr_net6):
  #print(best_guess)
...
def wrong_key_decryption(n, diff=(0x0040,0x0), nr=7, net = gohr_net7):
...
def bayesian_rank_kr(cand, emp_mean, m=gohr_m7, s=gohr_s7):
  global tmp_br
  n = len(cand)
  if (tmp_br.shape[1] != n):
      tmp_br = np.arange(2**14, dtype=np.uint16)
...
def bayesian_key_recovery(cts, net=gohr_net7, m = gohr_m7, s = gohr_s7, num_cand = 32, num_iter=5, seed = None):
  n = len(cts[0])
...
def test_bayes(index,cts,it=1, cutoff1=10, cutoff2=10, net=gohr_net7, net_help=gohr_net6, m_main=gohr_m7, m_help=gohr_m6, s_main=gohr_s7, s_help=gohr_s6, verify_breadth=None):
  n = len(cts[0])
  ...
  for j in range(it):
      ...
      if (vtmp > cutoff1):
        ...
        for i2 in l2:
          ...
          keys2,scores2,v2 = bayesian_key_recovery([c0a, c1a, c0b, c1b],num_cand=32, num_iter=5, m=gohr_m6,s=gohr_s6,net=net_help)
          vtmp2 = np.max(v2)
          if (vtmp2 > best_val):
            best_val = vtmp2; best_key = (keys[i2], keys2[np.argmax(v2)]); best_pod=i
            print('second best key is ', best_key, ' second score is ', best_val, ' cur index is ', i)
...
def test(index, n, ct_all, key_all, nr=11, num_structures=100, it=500, cutoff1=10.0, cutoff2=10.0, neutral_bits=[20,21,22,14,15,23], keyschedule='real',net=gohr_net7, net_help=gohr_net6, m_main=gohr_m7, s_main=gohr_s7,  m_help=gohr_m6, s_help=gohr_s6, verify_breadth=None):
  ...
  for i in range(n):
    ...
    guess, num_used = test_bayes(index=index,cts=ct,it=it, cutoff1=cutoff1, cutoff2=cutoff2, net=net, net_help=net_help, m_main=m_main, s_main=s_main, m_help=m_help, s_help=s_help, verify_breadth=verify_breadth)
