...
def make_train_data_with_joint_key(p0, p1, nr, group_size=2):
    n = p0.shape[0]
    assert n % group_size == 0
    num = n // group_size

    master_keys = np.frombuffer(urandom(num * 80), dtype=np.uint8).reshape(-1, 80) & 1
    new_master_keys = np.repeat(master_keys, group_size, axis=0)
    subkeys = expand_key(new_master_keys, nr)
...
def make_train_data_without_joint_key(p0, p1, nr):
    n = p0.shape[0]
    master_keys = np.frombuffer(urandom(n * 80), dtype=np.uint8).reshape(-1, 80) & 1
    subkeys = expand_key(master_keys, nr)
...
def make_dataset(n, nr, diff=0x9, group_size=2):
    assert n % group_size == 0
    num = n // 2
    ...
    X_raw_j = make_train_data_with_joint_key(p0, p1, nr=nr, group_size=group_size)
    m = X_raw_j.shape[1]
    X_j = X_raw_j.reshape((-1, group_size * m))
    # generate train data without joint key
    X_raw = make_train_data_without_joint_key(p0, p1, nr=nr)
    X = X_raw.reshape((-1, group_size * m))
    # generate labels
