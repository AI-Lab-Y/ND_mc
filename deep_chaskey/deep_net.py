...
def make_resnet(num_blocks=4, num_filters=64, num_outputs=1, d1=64, d2=64, word_size=32, ks=5, depth=5, reg_param=0.0001, final_activation='sigmoid'):
...
def train_chaskey_distinguisher(num_epochs, num_rounds=7, diff=(0x8400, 0x0400, 0, 0), depth=1):
    # create the network
    net = make_resnet(depth=depth, reg_param=10**-5)
    net.compile(optimizer='adam', loss='mse',metrics=['acc'])
    # generate training and validation data
    X, Y, X_eval, Y_eval = chk.create_train_test_dataset(n1=10**7, n2=10**6, x=num_rounds, y=0,
                                                         head=0, group_size=1, diff=diff)
