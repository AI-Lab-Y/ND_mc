...
def train_des_distinguisher(num_epochs, num_rounds=7, diff=(0x40080000, 0x04000000), depth=1):
    # create the network
    net = make_resnet(depth=depth, reg_param=10**-5)
    net.compile(optimizer='adam', loss='mse',metrics=['acc'])
    # generate training and validation data
    X, Y = des.make_dataset_with_group_size(n=10**7, nr=num_rounds, diff=diff, group_size=1)
    X_eval, Y_eval = des.make_dataset_with_group_size(n=10 ** 7, nr=num_rounds, diff=diff, group_size=1)
