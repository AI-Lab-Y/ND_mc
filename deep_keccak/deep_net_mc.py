...
def make_resnet(group_size=2, num_blocks=4, num_filters=32, num_outputs=1, d1=64, d2=64, word_size=64, ks=3, depth=5, reg_param=0.001, final_activation='sigmoid'):
...
def train_keccak_distinguisher(num_epochs, num_rounds=7, group_size=2, depth=1, diff=[(135, 0x80)]):
    #create the network
    net = make_resnet(group_size=group_size, depth=depth, reg_param=10**-4);
    net.compile(optimizer='adam', loss='mse', metrics=['acc']);
    #generate training and validation data
    X, Y = sha.make_dataset(200000, num_rounds, diff)
    X_eval, Y_eval = sha.make_dataset(20000, num_rounds, diff)

    a, b = X.shape[0], X.shape[1]
    a_eval, b_eval = X_eval.shape[0], X_eval.shape[1]

    X = X.reshape(a//group_size, b*group_size)
    Y = np.array([Y[i*group_size] for i in range(a//group_size)])
    X_eval = X_eval.reshape(a_eval//group_size, b_eval*group_size)
    Y_eval = np.array([Y_eval[i*group_size] for i in range(a_eval//group_size)])
    ...
    # save model
    net.save('./saved_model/' + str(num_rounds) + '_' + str(group_size) + '_mc_distinguisher.h5')
